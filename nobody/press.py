...
class Page:
    ...
    def __init__(self, dateline, headline, standfirst, reactions, asides,
        ...
        self.asides = asides            # [(headline, passage, deltas)] the world's beats
    ...
    def lines(self):
        ...
        for head, passage, deltas in self.asides:
            moved = ', '.join(f'{m} {d:+d}' for m, d in sorted(deltas.items()) if d)
            out.append(f'Meanwhile — {head}' + (f' ({moved})' if moved else ''))
            if passage:
                out.append('    ' + passage)
        return out


def compose(world, dateline, beat, deltas, asides=(), dithered=False):
    """The page for the player's `beat` (name, params) that moved the
    meters by `deltas`, with the world's `asides` [(beat, deltas)]."""
    name, params = beat
    layer = world.layer
    if dithered:
